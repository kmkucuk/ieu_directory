function [vp,data, D ] = SAB_Trig_konv(konvert, anfang, ende, anfang_krit, ende_krit, trig_kanal, kratz);
...
% 
% Beispiel : [vp,data, D] = SAB_Trig_konv('up023', 1500, 500, 250, 100, 14, 6);
% 
...
for k = 1: length(press)
    test=test+1
    try s = D(press(k)- (anfang-1) : press(k)+ende,:);
    sweep = cat(3,sweep,s);
    catch
              'sweep hat nicht geklappt'
              test
    end
end
% %abb

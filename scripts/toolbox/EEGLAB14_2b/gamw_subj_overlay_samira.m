...
answer = inputdlg({'Single subject overlay-plot: which dataset?', 'Which channels?','Plotlayout?', 'Epochsize?', 'y-axis limits', 'x-axis limits','x-ticks?','How many subjects/which subject?', 'Axis on or off'}, ...
    'Please define:', 1, {'GAMV_granny_ende_pat_Alpha_n10_d', '[1 2 3 4]', '[4 1]', '[-256 750]', ...
        '[-3 3]', '[-250 750]', '[0 250 500]','[1:10]', 'axis on'}); 
...
channelnames = eval(char([strrep(data,'_d', ''), '_p','.text_channel(chan,:)']));
...
%legend(eval(files)) 
saveas(gcf, char(answer(1,1)), 'tif'); 

function [S,HDR,time] = sread(HDR,NoS,StartPos,decimate)
...
%	$Id: sread.m,v 1.1 2008/04/01 20:20:37 nima Exp $
...
elseif strcmp(HDR.TYPE,'BDF'),
    ...
    if HDR.NS==0,
        ...
        if false%(HDR.AS.spb*nb<=2^22), % faster access, disables by nima since it causes a bug 
            S  = repmat(single(NaN),HDR.SPR*nb/decimate,length(HDR.InChanSelect));
            ...
            S      = repmat(single(NaN),nr,length(HDR.InChanSelect));
            while (count<nr);
        ...
        try
            ...
            for i = 1:length(HDR.InChanSelect)
                S(:,i) = S(:,i) - 2^24*(S(:,i)>=2^23);
            end;
        end;
    end
...
if ~HDR.FLAG.UCAL,
		...
		Calib = HDR.Calib;
		tmp   = S;
                S     = repmat(single(0),size(S,1),size(Calib,2));   % memory allocation

...
% training result: w/o cross-validation
r = test_sc(CC,D,MODE);
CC.TSD0 = bci4eval(r.output,TRIG,cl,min(T(:)),max(T(:))); 
...
%        fprintf(1,'\nX-V (%i/%i):',l, length(CL2));
        ...
        if size(cc.weights,1)<100,
        	if l==1,
	        	LDA = repmat(NaN,[length(CL2),size(cc.weights)]);  
	        end; 	
	        LDA(l,:,:) = cc.weights;
        end;
        end; 
...
if ~isempty(LDA),
	for l=1:size(LDA,3),
		[CC.ldaC0(:,:,l),NN] = covm(squeeze(LDA(:,:,l)),'D0');
	end;	
end; 
...
% cumulative output
tsd = reshape(tsd,[nc,length(cl),size(tsd,2)]); 
tsd(CC.TSD.T<=0,:,:)=0; 
cTSD = reshape(cumsum(tsd,1),[nc*length(cl),size(tsd,3)]); 
%cTSD = cTSD - repmat(cTSD(T0,:),size(cTSD,1),1); 
CC.cTSD   = bci4eval(cTSD, (0:length(cl)-1)'*nc, cl, 1, nc);
CC.cTSD.T = CC.TSD.T;


if sum(CC.TSD.optCMX(:))~=length(TRIG),
	fprintf(2,'number of classified trials (%i) differs from number of triggers (%i)\n',sum(CC.TSD.optCMX(:)),length(TRIG))
	CC.TSD.optCMX,
end; 

...
answer = inputdlg({'Single subject overlay-plot: which dataset?', 'Which channels?','Plotlayout?', 'Epochsize?', 'y-axis limits', 'x-axis limits','x-ticks?','How many subjects/which subject?', 'Axis on or off'}, ...
    'Please define:', 1, {'GAMV_Ende_gamma_n11_d', '[1 2 3 4]', '[4 1]', '[-256 750]', ...
        '[-0.5 0.5]', '[-250 750]', '[0 250 500]','[1:11]', 'axis on'}); 
...
channelnames = eval(char([strrep(data,'n11_d', 'n11'), '_p','.text_channel(chan,:)']));
...
figure
    for i = 1:size(chan,2); % Anzahl Plots (Kanaele)
        ...
        set(gca, 'ydir', 'reverse','color','none','xtick', x_tick, 'fontsize', 18); xlim([x(1) x(2)]); ylim([y(1) y(2)]); y = get(gca, 'ylim');  x = get(gca, 'xlim');
        line([0 0], [y(1) y(2)], 'color', 'k'); text(x(1)+100, y(1), channelnames(i,1:7), 'fontsize', 18); box off; 
...
%legend(eval(files)) 
saveas(gcf, ([char(answer(1,1)),'pat']), 'tif'); 

...
%       the portMarker_Trial value below 
        ...
        if balancePressedKeys
            response_bytes       = flip(response_bytes); % counterbalanced response keys (1)
            response_byte_names  = flip(response_byte_names); % counterbalanced response key descriptions (1)
        end
        ...
        callTextInstruction_Port(window,instructionText,xCenter, yCenter,portVariables)
        ...
        for flowIteration=flowVector
           ...
           %% register all stimulus onsets into stimOnsetRegistery
            ...
            previousPress       = Pressed ==1; % if there is an ongoing press from the previous display  
            ...
            while 1
                ...
                if (reversalIteration>=1 || (checkEndogenousTask && presentation_durations == 2) || FCRTT_keyReg)
                        ...
                        if  previousPress
                            ...
                            % do nothing, let script process the previous press
                                
                        elseif registeredKeys == 0 
                        ...
                        if Pressed && registeredKeys == 0  % if there is a press and it is the first one since the reversal
                            %% CHECK BUTTON PRESS AND REGISTER PARAMETERS 
                                ...
                                checkValidKeys=(keyIndex==response_bytes(1) || keyIndex==response_bytes(2)); % if pressed keys are either left or right buttons and not smth else.
                                ...
                                if enablePressDuration  % continue with press release algorithm
                                    ...
                                    if checkValidKeys
                                        ...
                                        if checkEndogenousTask && presentation_durations ~= 1
                                            ...
                                            if response_bytes(1)==keyIndex
                                                reversalTo = 'horizontal';
                                                reversalFrom = 'vertical';
                                            elseif response_bytes(2)==keyIndex
                                                reversalTo = 'vertical';

...
prompt = {'Subjects ID:', 'age', 'gender',...
    'Handedness (Left(0)/Right(1))','Reverse Buttons? y(1) n(0)'}; % adjust-color position pairs 
defaults = {'subid_simon', '25', 'Male','0','0'};
answer = inputdlg(prompt, 'Simon Task Info', 2, defaults);
[subid, subage, gender, handedness,reverseButton] = deal(answer{:}); % all input variables are strings

% STIMULUS PRESENTATION: INSTRUCTION
%
% open an on screen window
[window, windowRect] = PsychImaging('OpenWindow', screenNumber, grey); %,[700 0 1000 600]
...
cd('D:\MatlabDirectory\experiments\simon_task\instructions')
...
cd('D:\MatlabDirectory\experiments\simon_task\stimuli')
% fixation cross
...
stimulusVector              = {cong_left_texture,cong_right_texture,incong_left_texture,incong_right_texture};
% stimulus types
stimulusTypeVector          = {'comp_left','comp_right','incomp_left','incomp_right'};
% responses correspond to each stimulus type, columnwise
responseVector              = {one_pressByte,two_pressByte,one_pressByte,two_pressByte};

if num2str(reverseButton) == 1
    responseVector          = flip(responseVector);
end

% stimulus condition port signals (cong, incong; 20,21 respectively)
conditionPortVector          = {20,20,21,21};
% detailes stimulus port signals (cong-left, cong-right, incong-left, incong-right; 30,31,35,36 respectively)
stimulusPortVector           = {30,31,35,36};
% condition port vector 
stimulusConditionVector      = {'compatible','compatible','incompatible','incompatible'};
...
cd('D:\MatlabDirectory\experiments\simon_task\data')
...
dataFileName = [subid,'_simon_',dateTimeStr,'.xlsx'];

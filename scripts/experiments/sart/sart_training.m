...
practiceTrials = experimental_trials(:,18:37); % define practice trials 
experimental_trials = practiceTrials;
practiceTrialCount = length(practiceTrials);
% experimental_trials(:,40:end) = []; 
motor_responseTable             = cell(length(experimental_trials),6);
motor_responseTable(1,:)        = {'Trial_No','Stimulus_Type','Stimulus_Frequency','Accuracy','RT','stimTimeInSeconds'};
...
%% first instruction
callImageInstruction_Port(window,instructionInitial_Texture,portVariables)
...
for k = 1:length(experimental_trials)
   ...
   %% instructions and transition from practice to experimental trials
    if k == 1
        interStimInterval = 2; % 2 sec ISI during practice trials
        % initiation of practice trials
        % send stimulus marker to brain vision recorder 
        sendParallelSignal(portAddress,50,ioObj) % S50 = start of practice trials
        WaitSecs(.005);
        endParallelSignal(portAddress,ioObj)        
        
        
    elseif k == 1+practiceTrialCount        
...
%     disp('stop tock')    
    ...
    if k == 1 
        % register first practice stim onset in seconds
        firstOnset = stimTime;      
    end
    ...
    timeElapsed = GetSecs() - stimTime;
        ...
        if timeElapsed > interStimInterval % 1 sec during experimental, 2 sec during practice trials 
            ...
            motor_responseTable{k+1,1} = k; % trial number
            motor_responseTable{k+1,2} = experimental_trials{3,k}; % target or non target
            motor_responseTable{k+1,3} = experimental_trials{2,k}; % frequency of stimuli
            motor_responseTable{k+1,4} = respAccuracy; % keyboard key name
            motor_responseTable{k+1,5} = reactionTime; % estimated RT
            motor_responseTable{k+1,6} = stimulusOnset; % release duration
            ...
            WaitSecs(.005);
            endParallelSignal(portAddress,ioObj)
            if interStimInterval > 1
                % if these are practice trials (more than 1 sec ISI)
                Screen('DrawTexture',window,feedbackTexture);
                Screen('Flip',window);   
                WaitSecs(1); % display feedback for 500 ms
                DrawFormattedText(window,fixationCross,'center', 'center',[1 1 1],120,[],[],1.25);
                Screen('Flip',window);
                WaitSecs(1);
            end
...
expEnd = GetSecs;
experimentDuration = expEnd - firstOnset;

% initiation of experimental trials and end of practice trials
sendParallelSignal(portAddress,51,ioObj) % S51 = end of practice trials
WaitSecs(.005);
endParallelSignal(portAddress,ioObj)   

% practice trial finished instruction 
callImageInstruction_Port(window,instructionPracticeOver_Texture,portVariables)        

        
timePointCorrectedOnsets    =   (round((stimOnsetRegistery-firstOnset)/samplingFreqCorrection,3)+.001)*1000; % correct screen onset times into sampling rate corrected time points (1 256... 1325 etc.)        
...
dataFolder =['D:\MatlabDirectory\AllScripts\ZehraUlgen\SART\rawDataFolder\' subid,'_training'];
mkdir(dataFolder);
cd(dataFolder);
...
pInfoFile = fopen([subid '_training_info.txt'],'w');
fprintf(pInfoFile,participantInfo);
writetable(table_motor,[subid '_behavioralDataTraining.txt'])
writetable(table_stim,[subid '_stimulusTableTraining.txt'])

function [] = f_PlotEEG_BrainNetwork(nch, ijw, w_scal, n_features, n_scal, cbtype,zlimits)
% 
...
%     disp(nch);                          %Displaying number of nodes (activate it for testing purposes)
    ...
    return;
elseif nargin == 3 || nargin == 4                   %Previous + Matrix and the Scaling of Weights
...
elseif nargin == 6 || nargin == 7                      %Previous + Nodes' attributes and Node Scaling
    w_atribut = ijw(:, 3);  
    w_scaling = w_scal;
...
cmaps(28:end,:)=[];
values = flip(linspace(zlimits(1),zlimits(2),length(cmaps)));

w_atribut_new=(sort(w_atribut_new));
assignin('base','w_atribut_new',w_atribut_new);

indx = flip(dsearchn(values',w_atribut_new));
assignin('base','indx',indx);

RGBlinks = flip(cmaps(indx,:));
assignin('base','RGBlinks',RGBlinks);
% RGBlinks = squeeze(double2rgb(w_atribut_new, cmaps,[3 4.3288]));
...
switch n_scaling
    case 'n_fixed'
        ...
        RGBnodes = squeeze(double2rgb(n_atribut_new, colormap([1 1 1])));
...
plot(lay.outline{1}(:,1), lay.outline{1}(:,2), 'g', 'LineWidth',3, 'color', [0,0,0]./255)
plot(lay.outline{2}(:,1), lay.outline{2}(:,2), '--k', 'LineWidth',3)
plot(lay.outline{2}(:,1), lay.outline{2}(:,2), 'g', 'LineWidth',3, 'color', [0,0,0]./255)
plot(lay.outline{3}(:,1), lay.outline{3}(:,2), '--k', 'LineWidth',3)
plot(lay.outline{3}(:,1), lay.outline{3}(:,2), 'g', 'LineWidth',3, 'color', [0,0,0]./255)
plot(lay.outline{4}(:,1), lay.outline{4}(:,2), '--k', 'LineWidth',3)
plot(lay.outline{4}(:,1), lay.outline{4}(:,2), 'g', 'LineWidth',3, 'color', [0,0,0]./255)
...
for lk = 1 : size(ijw, 1)           %along all links
    ...
    line(xynodes(:, 1), xynodes(:, 2), 'LineWidth', 4, 'Color', RGBlinks(lk, :));
    % % %     line(xynodes(:, 1), xynodes(:, 2), 'LineWidth', ijw(lk, 3), 'Color', [.8 .8 .1]); % links de un solo color
    
    for nd = 1 : 2                                  %for the two nodes of a link
        plot(xynodes(nd, 1), xynodes(nd, 2), 'o', 'MarkerSize',4, ... %, 5*pi.*n_atribut_new(ijw(lk,nd))
            'MarkerEdgeColor','k', 'MarkerFaceColor', RGBnodes(ijw(lk,nd), :), 'LineWidth',4);
%         text(xynodes(nd, 1)-0.017, xynodes(nd, 2)-0.006, labels{ijw(lk,nd)}, 'fontsize', 15);
